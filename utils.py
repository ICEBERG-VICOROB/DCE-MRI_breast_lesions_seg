...
class MRI_DataPatchLoader(Dataset):
    ...
    def get_candidate_voxels(self, input_mask, label_mask, roi_mask):
        ...
        if self.sampling_type == 'hybrid':
            ...
            print("number of initial pos samples",len(x))
            ...
            '''x = np.resize(x, self.num_pos_samples)
            y = np.resize(x, self.num_pos_samples)
            z = np.resize(x, self.num_pos_samples)'''
            print("number of extended pos samples",len(x))
            ...
            for x_v, y_v, z_v in zip(x, y, z):
                if sampled_mask[x_v, y_v, z_v] == 1:
                    s+=1
                sampled_mask[x, y, z] = 1
            print("number of duplicate samples", s)
            print("number of positive samples", np.sum(sampled_mask>0))
            
            # negative samples
            brain_voxels = np.stack(np.where(roi_mask > 0), axis=1)
            print("number of initial neg samples", len(brain_voxels))
            i=0
            m=0
            for voxel in np.random.permutation(brain_voxels)[:self.num_pos_samples]:
                if (sampled_mask[voxel[0], voxel[1], voxel[2]] == 1):
                    m+=1
                else:
                    sampled_mask[voxel[0], voxel[1], voxel[2]] = 1
                ...
                #sampled_mask[voxel[0], voxel[1], voxel[2]] = 1
                i+=1
            print("number of final neg samples",i)
            print("number of negative samples",len(np.random.permutation(brain_voxels)[:self.num_pos_samples]))
            print("number of mutual samples",m)
            print("number of total samples",np.sum(sampled_mask>0))
...
def get_inference_patches(scan_path, input_data, roi, patch_shape, step, normalize=True, additional_path=''):
    """
...
def get_data_channels(image_path,
    ...
    # append the additional input image path if exist
    if not additional_path=='':
        current_scan = additional_path
        patches, _ = get_input_patches(current_scan,
                                       ref_voxels,
                                       patch_shape,
                                       step,
